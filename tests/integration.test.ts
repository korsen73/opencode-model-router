...
// CASE I4: candidate chain bounded by CANDIDATE_CHAIN_MAX (4) and no duplicates.
// The INJECTED models[] array (primary-excluded) is separately clamped to 3.
test("CASE I4: candidate chain <= CANDIDATE_CHAIN_MAX (4) and no duplicates", async () => {
  const cfg = baseConfig();
  await seedRegistry();
  await clearCooldown("openrouter");
  for (let r = 0; r < 10; r++) {
    const d = await decide(cfg, "coder");
    assert.ok(d.chain.length <= 4, `chain too long: ${d.chain.length}`);
    assert.equal(new Set(d.chain).size, d.chain.length, "chain has duplicates");
  }
});

// CASE I4b: a config value > 3 is bounded by CANDIDATE_CHAIN_MAX (4).
test("CASE I4b: config maxFreeModelsPerChain > 3 is bounded to CANDIDATE_CHAIN_MAX (4)", async () => {
  const cfg = { ...baseConfig(), maxFreeModelsPerChain: 5 };
  await seedRegistry();
  await clearCooldown("openrouter");
  const d = await decide(cfg, "coder");
  assert.ok(d.chain.length <= 4, `config=5 should cap at 4, got ${d.chain.length}`);
});
